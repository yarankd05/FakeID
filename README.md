...
├── notebooks/
│   ├── feature3_eval.ipynb    # Feature 3 evaluation - YOLO + EfficientNet metrics
│   ├── feature1_eval.ipynb    # Feature 1 evaluation - face verification metrics
│   └── feature2_eval.ipynb    # Feature 2 evaluation - age estimation metrics
└── requirements.txt
...
All responses follow the envelope format: {"success": true, "data": {...}, "error": null}
...
Input Image
|
├─ is_scan() — A4 aspect ratio + resolution >= 2400px long side
|
├── SCAN PATH
|   Layer 1: Canny contour crop (94/100 4-corner perspective correction)
|   Layer 2: YOLOv8n zone detection on original image
|   Layer 3: Geometric analysis vs spain.json (tolerance 0.08)
|   Layer 4: EfficientNet-B0 classifier on original image
|
└── PHOTO PATH
Layer 1: YOLO-World zero-shot card detection and crop
Layer 2: YOLOv8n zone detection on original image
Layer 3: Geometric analysis vs spain_photo.json (tolerance 0.25)
Layer 4: EfficientNet-B0 classifier on original image
|
